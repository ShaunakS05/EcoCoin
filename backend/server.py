...
def futureData(tokenName: str=Form()):
    if tokenName == "Carbon":
        ...
        forecast_json = forecast[['ds', 'yhat1']].rename(columns={'ds': 'date', 'yhat1': 'open'})
        forecast_json['date'] = forecast_json['date'].astype(str)
        forecast_json_output = forecast_json.to_dict(orient='records')
        nested_forecast_json_output = {
        "prices": forecast_json_output
        }
        return nested_forecast_json_output
